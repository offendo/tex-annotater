...
export function LinkMenu(props: LinkMenuProps) {
    ...
    const [otherFileAnnotations, setOtherFileAnnotations] = useState<
        TextSpan[]
    >([]);
    ...
    async function queryAutoLinks(text: string) {
        try {
            const response = await fetch(`/api/definition?query=${text}`, {
                mode: "cors",
            });
            const res = await response.json();
    ...
    useEffect(() => {
        ...
        if (showAutoLinks && props.selectedAnnotation.tag == "reference") {
            queryAutoLinks(props.selectedAnnotation.text);
        } else {
    ...
    const makeRow = (
        ...
        return (
            <tr
                key={crypto.randomUUID()}
                className={
                    selected ? "link-menu-item link-selected" : "link-menu-item"
                }
            >
                <td>
                    {" "}
    ...
    const makeAutoLinkRow = (annotation: any, index: number) => {
        return (
            <tr
                key={crypto.randomUUID()}
                className={"link-menu-item link-auto-link"}
            >
                <td>
                    <IconButton
                        size="small"
        ...
        >
            ...
            <table>
                ...
                <tbody>
                    ...
                    {filterSearch(nonLinkedAnnos, query).map(
                        (annotation, index) =>
                            makeRow(annotation, index, false),
                    )}
                </tbody>
            </table>
        </div>
